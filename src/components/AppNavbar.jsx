...
export default function AppNavbar() {
  ...
  // Shared nav link button used in both desktop and mobile
  const NavButton = ({ link, isAdminLink = false }) => {
    const active    = isActive(link.path);
    const textColor = isAdminLink ? nav.adminText : nav.activeText;
    const activeBg  = isAdminLink ? nav.adminBg   : nav.activeBg;
    ...
    return (
      <button
        ...
        style={{
          ...
          color:      active ? textColor : nav.text,
          transition: "all 0.2s",
          whiteSpace: "nowrap",
          width:      "100%",
          textAlign:  "right",
      ...
      >
        ...
        {active && (
          <span style={{
            ...
            height:    "4px",
            borderRadius: "50%",
            background: isAdminLink ? "#2E8B57" : "#C9A84C",
          }} />
        )}
      </button>
    );
  ...
  return (
    <>
      {/* ── INLINE STYLE for breakpoint ── */}
      <style>{`
        .gantra-nav-links { display: flex; }
        .gantra-hamburger  { display: none; }
        @media (max-width: 900px) {
          .gantra-nav-links { display: none; }
          .gantra-hamburger  { display: flex; }
        }
      `}</style>
      ...
      >
        ...
        >
          ...
          {/* ── DESKTOP NAV LINKS ── */}
          <div className="gantra-nav-links" style={{
            alignItems:     "center",
            gap:            "2px",
            flex:           1,
            ...
            {isAdmin && (
              <>
                ...
                {ADMIN_LINKS.map((link) => (
                  <NavButton key={link.path} link={link} isAdminLink />
                ))}
              </>
            )}
          </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "8px", flexShrink: 0 }}>
            ...
            <button
              className="gantra-hamburger"
              onClick={() => setMobileOpen((v) => !v)}
              style={{
                alignItems: "center", justifyContent: "center",
                width: "36px", height: "36px", borderRadius: "10px",
                border: `1px solid ${nav.iconBorder}`, background: nav.iconBtn,
                color: nav.iconColor, cursor: "pointer",
                transition: "all 0.2s", flexShrink: 0,
              }}
            >
        ...
        {mobileOpen && (
          ...
          >
            ...
            {isAdmin && (
              <>
                <div style={{
                  height: "1px", background: nav.divider,
                  margin: "8px 0",
                }} />
                <p style={{
                  fontSize: "0.7rem", fontWeight: "700",
                  color: "#2E8B57", letterSpacing: "0.06em",
